...
def plot_missing_values(df):
    ...
    fig = px.bar(
        missing[missing > 0], title="Missing Values per Feature", labels={"value": "Missing Count", "index": "Feature"}
    )
    st.plotly_chart(fig, width="stretch")
...
def plot_histograms(df):
    ...
    for col in numerical_cols:
        fig = px.histogram(
            df, x=col, nbins=30, marginal="box", title=f"Histogram of {col}", color_discrete_sequence=["#636EFA"]
        )
...
def plot_boxplots(df, target_col=None):
    ...
    for col in numerical_cols:
        if target_col and target_col in df.columns:
            ...
            fig = px.box(df, y=col, points="suspectedoutliers", title=f"Boxplot of {col}")
...
def plot_pairplot(df):
    ...
    fig = px.scatter_matrix(
        df[numerical_cols], title="Interactive Pairplot", dimensions=numerical_cols, color=numerical_cols[0]
    )
