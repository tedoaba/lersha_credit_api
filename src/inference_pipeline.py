...
from src.infer_utils import (
    preprocessing_categorical_features, get_candidate_data, load_prediction_model, predict_single_sample_data, 
    define_shap_explainer, build_contribution_table, generate_shap_for_sample, get_featured_data, apply_feature_engineering,
...
def run_predictions(model_name, original_data, selected_data, feature_column, target_column):
    ...
    with mlflow.start_run(run_name=run_name, nested=True):
        ...
        for idx in range(len(sample_encoded)):
            ...
            if model_name == "model_34":
                ...
                _, shap_values = define_shap_explainer(model, row)
            
                generate_shap_for_sample(row, shap_values, model_name)
